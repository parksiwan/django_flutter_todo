...
class TaskProvider extends ChangeNotifier {
  ...
  void addTodo(Task task) async {
    final response = await http.post(
      //"http://192.168.122.218:8000/todo",
      //"http://localhost:8000/todo",
      "http://192.168.0.6:8000/todo",
  ...
  void toggleTodo(Task task) async {
    ...
    final response = await http.patch(
      //"http://192.168.122.218:8000/todo/${task.id}",
      "http://192.168.0.6:8000/todo/${task.id}",
      headers: {"Content-Type": "application/json"},
      body: json.encode(task),
    );
  ...
  fetchTaks() async {
    //final response = await http.get("http://192.168.122.218:8000/todo");
    final response = await http.get("http://192.168.0.6:8000/todo");
    if (response.statusCode == 200) {
